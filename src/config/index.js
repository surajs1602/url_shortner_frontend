...
// Both can be overridden at runtime via the Settings modal (stored in localStorage).
export const DEFAULT_BASE_URL = import.meta.env.VITE_API_BASE_URL || '';
export const DEFAULT_API_KEY  = import.meta.env.VITE_API_KEY      || '';

// Frontend public URL — used to build shareable short links (/go/:id).
// Set VITE_APP_URL in .env to your deployed frontend URL (e.g. https://shortit.vercel.app).
// Falls back to window.location.origin at runtime so local dev works without any config.
...
// Demo data seed count (days of fake history generated per link)
